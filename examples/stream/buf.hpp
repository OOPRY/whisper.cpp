...
{
public:
	static const std::size_t kCheapPrepend = 1024;
	static const std::size_t kInitialSize = 512;

	explicit Buf(std::size_t initialSize = kInitialSize)
		:
	...
	int32_t readInt32()
	{
		...
		///
	/// Read T from network endian
	///
	/// Require: buf->readableBytes() >= sizeof(T)
	template<typename T>
	T read()
	{
		T const* ptr = reinterpret_cast<T const*>(peek());
